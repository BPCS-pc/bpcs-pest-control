...
const App = () => {
  ...
  const renderSafeText = (text) => text ? String(text) : "";
  ...
  // 핵심 검색 로직: 오직 '이름'에서만 찾기 (주소/번호 제외)
  ...
  if (loading && customers.length === 0) {
    return <div className="flex flex-col items-center justify-center min-h-screen bg-white"><div className="w-12 h-12 border-4 border-blue-900 border-t-transparent rounded-full animate-spin mb-4"></div><p className="font-black text-blue-900 uppercase">데이터 동기화 중...</p></div>;
  }
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24 text-left overflow-x-hidden">
      ...
      <nav className="bg-white border-b sticky top-0 z-30 p-4 flex justify-between items-center shadow-sm print:hidden">
        <div className="flex items-center gap-2 font-black text-xl text-blue-900 cursor-pointer" onClick={() => {setCurrentView('dashboard'); setSearchTerm('');}}>
          <div className="bg-blue-900 p-1.5 rounded-xl text-white"><ShieldCheck size={22}/></div>
          BPCS 방역특별시
        </div>
      ...
      <main className="max-w-4xl mx-auto p-5 print:p-0">
        ...
        {currentView === 'dashboard' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="relative">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-5 flex items-center text-slate-300"><X size={20}/></button>
              )}
              ...
              {searchTerm.trim() !== "" && (
                <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-[2rem] shadow-2xl border border-blue-50 z-50 max-h-80 overflow-y-auto p-2">
                  <div className="px-5 py-3 text-[10px] font-black text-blue-900 uppercase border-b border-slate-50 mb-1 tracking-widest">이름 검색 결과 ({filteredCustomers.length})</div>
                  {filteredCustomers.length > 0 ? (
                    ...
                    <div className="p-10 text-center text-slate-400 font-bold italic">검색어와 일치하는 이름이 없습니다.</div>
                  )}
                </div>
              )}
            </div>
        ...
        {currentView === 'reports_history' && (
          <div className="space-y-6 animate-in fade-in">
             ...
             <input type="text" placeholder="거래처명으로 기록 찾기..." className="w-full p-5 rounded-2xl border-none shadow-inner bg-white font-bold outline-none" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
             <div className="space-y-3 pb-20">
               {reports.filter(r => renderSafeText(r.customerName).includes(searchTerm)).map(r => (
                 <div key={r.id} className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100 flex justify-between items-center active:bg-slate-50 transition-all" onClick={() => { setFormData(r); setCurrentView('report_view'); }}>
                   <div className="text-left">
                     <div className="text-[10px] font-black text-blue-900 bg-blue-50 px-2 py-1 rounded-full inline-block mb-1">{renderSafeText(r.date)}</div>
             ...
             <div id="report-area" className="bg-white p-10 sm:p-20 border border-slate-100 shadow-2xl rounded-[3rem] text-left print:shadow-none print:border-none print:p-0">
               ...
               <div className="space-y-16">
                 ...
                 <section>
                   ...
                   <div className="bg-slate-50 p-10 rounded-[2.5rem] min-h-[250px] leading-[1.8] font-bold text-xl text-slate-800 whitespace-pre-wrap">{renderSafeText(formData.workContent) || "기록된 내용이 없습니다."}</div>
                 </section>
               </div>
             </div>
           </div>
        ...
        {currentView === 'edit' && (
          <div className="space-y-6 animate-in slide-in-from-bottom">
            ...
            <button onClick={() => setCurrentView('dashboard')} className="w-full text-slate-400 font-bold py-2">취소하고 돌아가기</button>
          </div>
        )}

        {/* --- 거래처 상세 및 등록 등 나머지 뷰 생략 (기존과 동일하되 검색 최적화 반영) --- */}
        ...
        {currentView === 'customer_detail' && selectedCustomer && (
          <div className="space-y-6 animate-in slide-in-from-right"><button onClick={() => {setCurrentView('dashboard'); setSearchTerm('');}} className="flex items-center gap-1 text-slate-400 font-bold mb-2"><ChevronLeft size={20}/> 대시보드로</button><div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50"><h2 className="text-4xl font-black text-slate-900 mb-6">{renderSafeText(selectedCustomer.name)}</h2><div className="space-y-3 text-slate-600 font-bold"><p className="flex items-center gap-3"><Phone size={18} className="text-blue-900"/> {renderSafeText(selectedCustomer.phone) || "미등록"}</p><p className="flex items-center gap-3"><MapPin size={18} className="text-blue-900"/> {renderSafeText(selectedCustomer.address) || "미등록"}</p></div></div><div className="bg-blue-900 p-7 rounded-3xl shadow-xl text-white text-left"><h4 className="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-3 flex items-center gap-2"><BookOpen size={14}/> 내부 관리 메모</h4><p className="font-bold text-sm leading-relaxed whitespace-pre-wrap">{renderSafeText(selectedCustomer.privateMemo) || "기록된 지침이 없습니다."}</p></div><button onClick={() => { setFormData({...initialReportForm, customerName: selectedCustomer.name}); setIsEditMode(false); setCurrentView('edit'); }} className="w-full bg-blue-100 text-blue-900 p-6 rounded-2xl font-black flex items-center justify-center gap-2 mt-4">이 거래처로 새 작업 작성하기 <ArrowRight size={18}/></button></div>
        )}
